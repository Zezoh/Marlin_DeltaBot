...
extern "C" {
  ...
  inline void clear_wait_for_filament() {
    if (printer_states.activity_state == ACTIVITY_CHANGING_FILAMENT
        && runout.filament_present_debounced()) {
      delay(100);
...
void manage_one_led() {
  ...
  inline void wait_for_filament_insert() {
    KEEPALIVE_STATE(PAUSED_FOR_USER);
    wait_for_filament = true; // Sensor clear will reset this.
    while (wait_for_filament) {
      idle(true);
      KEEPALIVE_STATE(IN_HANDLER);
    }
  ...
  inline void auto_load_filament() {
    ...
    load_filament(FILAMENT_CHANGE_SLOW_LOAD_LENGTH, FILAMENT_CHANGE_FAST_LOAD_LENGTH);
  }

  inline void auto_unload_filament() {
    unload_filament(-FILAMENT_CHANGE_UNLOAD_LENGTH);
  }
#endif
