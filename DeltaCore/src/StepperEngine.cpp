...
StepperEngine::StepperEngine()
  : queue_(nullptr), dda_(), current_block_{{0,0,0},0,0}, mode_(MODE_IDLE),
...
void StepperEngine::enableMotors() {
  noInterrupts();
  setEnableFast(true);
  interrupts();
}

void StepperEngine::disableMotors() {
  if (!idle()) return;
  noInterrupts();
  setEnableFast(false);
  interrupts();
...
bool StepperEngine::endstopTriggered(const uint8_t axis) const {
  ...
  // Match Marlin semantics: triggered when READ(pin) != ENDSTOP_INVERTING.
...
void StepperEngine::stopTimerFromISR() {
  TIMSK1 = 0;
  allStepsInactive();
}
...
bool StepperEngine::loadNextMotionBlock() {
  ...
  if (!dda_.begin(current_block_.steps)) return false;
  block_active_ = true;
  return true;
}
...
bool StepperEngine::motionBusy() const {
  return mode_ == MODE_MOTION || block_active_ || (queue_ && !queue_->empty());
}

bool StepperEngine::idle() const {
  return mode_ == MODE_IDLE && !block_active_ && (!queue_ || queue_->empty());
}
...
bool StepperEngine::startHomeSeek(const bool slow) {
  ...
  queue_->clearUnsafe();
  setEnableFast(true);
  home_kind_ = HOME_KIND_SEEK;
  home_result_ = HOME_RESULT_RUNNING;
  home_active_axes_ = 0x07;
  home_events_done_ = 0;
  ...
  applyDirectionBits(0x07); // positive tower direction = toward MAX endstops
  mode_ = MODE_HOME;
  startTimer(cfg::STARTUP_EVENT_TICKS);
  interrupts();
  return true;
...
void StepperEngine::clearHomeResult() {
  noInterrupts();
  if (home_result_ != HOME_RESULT_RUNNING) home_result_ = HOME_RESULT_NONE;
  interrupts();
}
...
void StepperEngine::motionISR() {
  if (!block_active_) {
    ...
    OCR1A = current_block_.interval_ticks;
    return; // direction setup interval before first pulse
  }
  ...
  if (!dda_.active()) {
    block_active_ = false;
    MotorBlock next;
    if (queue_ && queue_->popFromISR(next) && dda_.begin(next.steps)) {
      current_block_ = next;
      block_active_ = true;
      pending_direction_bits_ = next.direction_bits;
      direction_pending_ = true; // applied only after current STEP pulse goes inactive
      OCR1A = next.interval_ticks;
    }
    else {
      OCR1A = cfg::STARTUP_EVENT_TICKS;
    }
...
void StepperEngine::homeISR() {
  if (home_kind_ == HOME_KIND_SEEK) {
    ...
    if (!remaining) {
      finishHomeFromISR(true);
      return;
    }
    if (home_events_done_ >= home_event_limit_) {
      fault_ = FAULT_HOME_TRAVEL;
      setEnableFast(false);
      finishHomeFromISR(false);
      return;
    }

    pulseAxes(remaining, true);
    ++home_events_done_;
    OCR1A = home_interval_ticks_;
    return;
  }
...
void StepperEngine::onCompareB() {
  ...
  if (direction_pending_) {
    applyDirectionBits(pending_direction_bits_);
...
void StepperEngine::emergencyStop(const FaultCode code) {
  ...
  mode_ = MODE_IDLE;
  home_kind_ = HOME_KIND_NONE;
  home_result_ = HOME_RESULT_FAILED;
  fault_ = code;
  setEnableFast(false);
  interrupts();
}
...
void StepperEngine::getMotorPositionSteps(int32_t steps[3]) const {
  noInterrupts();
  for (uint8_t axis = 0; axis < 3; ++axis) steps[axis] = motor_position_steps_[axis];
  interrupts();
}
...
ISR(TIMER1_COMPA_vect) {
  if (deltacore::StepperEngine::instance()) deltacore::StepperEngine::instance()->onCompareA();
}

ISR(TIMER1_COMPB_vect) {
  if (deltacore::StepperEngine::instance()) deltacore::StepperEngine::instance()->onCompareB();
}
