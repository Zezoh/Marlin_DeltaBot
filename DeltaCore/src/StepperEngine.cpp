...
StepperEngine::StepperEngine():queue_(nullptr),block_a_{},block_b_{},active_block_(&block_a_),prefetch_block_(&block_b_),prefetch_valid_(false),mode_(MODE_IDLE),block_active_(false),motors_enabled_(false),fault_(FAULT_NONE),motor_position_steps_{0,0,0},completed_step_events_(0),blocks_loaded_(0),queue_empty_stops_(0),timer_guard_hits_(0),real_steps_{0,0,0},pulse_overlap_events_(0),direction_overlap_events_(0),min_interval_ticks_(0xFFFF),max_interval_ticks_(0),max_isr_entry_ticks_(0),first_guard_requested_ticks_(0),first_guard_tcnt_ticks_(0),last_guard_requested_ticks_(0),last_guard_tcnt_ticks_(0),step_out_{nullptr,nullptr,nullptr},dir_out_{nullptr,nullptr,nullptr},enable_out_{nullptr,nullptr,nullptr},endstop_in_{nullptr,nullptr,nullptr},step_mask_{0,0,0},dir_mask_{0,0,0},enable_mask_{0,0,0},endstop_mask_{0,0,0},active_pulse_axes_(0),direction_pending_(false),pending_direction_bits_(0),dda_accum_{0,0,0},event_index_(0),timing_accum_(0),home_kind_(HOME_KIND_NONE),home_result_(HOME_RESULT_NONE),home_active_axes_(0),home_events_done_(0),home_event_limit_(0),home_interval_ticks_(0){}
...
void StepperEngine::snapshotStats(StepperStats&s)const{noInterrupts();s.virtual_events=completed_step_events_;s.blocks_loaded=blocks_loaded_;s.queue_empty_stops=queue_empty_stops_;s.timer_guard_hits=timer_guard_hits_;for(uint8_t a=0;a<3;++a)s.real_steps[a]=real_steps_[a];s.phase_anchors=s.phase_boundary_corrections=s.phase_faults=0;s.pulse_overlap_events=pulse_overlap_events_;s.direction_overlap_events=direction_overlap_events_;s.min_interval_ticks=min_interval_ticks_==0xFFFF?0:min_interval_ticks_;s.max_interval_ticks=max_interval_ticks_;s.max_isr_entry_ticks=max_isr_entry_ticks_;s.first_guard_requested_ticks=first_guard_requested_ticks_;s.first_guard_tcnt_ticks=first_guard_tcnt_ticks_;s.last_guard_requested_ticks=last_guard_requested_ticks_;s.last_guard_tcnt_ticks=last_guard_tcnt_ticks_;interrupts();}
void StepperEngine::clearStats(){noInterrupts();completed_step_events_=blocks_loaded_=queue_empty_stops_=timer_guard_hits_=0;pulse_overlap_events_=direction_overlap_events_=0;for(uint8_t a=0;a<3;++a)real_steps_[a]=0;min_interval_ticks_=0xFFFF;max_interval_ticks_=max_isr_entry_ticks_=0;first_guard_requested_ticks_=first_guard_tcnt_ticks_=last_guard_requested_ticks_=last_guard_tcnt_ticks_=0;interrupts();}
...
void StepperEngine::scheduleMotionInterval(uint16_t t){if(t<cfg::MIN_EVENT_INTERVAL_TICKS)t=cfg::MIN_EVENT_INTERVAL_TICKS;if(t>cfg::MAX_EVENT_INTERVAL_TICKS)t=cfg::MAX_EVENT_INTERVAL_TICKS;const uint16_t requested=t;uint16_t e=TCNT1;uint32_t earliest=uint32_t(e)+cfg::TIMER_ISR_GUARD_TICKS;if(uint32_t(t)<=earliest){if(!timer_guard_hits_){first_guard_requested_ticks_=requested;first_guard_tcnt_ticks_=e;}last_guard_requested_ticks_=requested;last_guard_tcnt_ticks_=e;t=earliest>cfg::MAX_EVENT_INTERVAL_TICKS?cfg::MAX_EVENT_INTERVAL_TICKS:uint16_t(earliest);++timer_guard_hits_;}if(t<min_interval_ticks_)min_interval_ticks_=t;if(t>max_interval_ticks_)max_interval_ticks_=t;OCR1A=t;}
